...
@section('content')
	...
	<div class="card shadow">
            ...
            @endif
                ...
                <form action="{{ url('/workers/'.$workers->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="name">Nombre del contacto</label>
                        <input type="text" name="name" class="form-control" value="{{ old('name', $workers->name) }}" required>
                    </div>

                    <div class="form-group">
                        <label for="last_name">Apellido</label>
                        <input type="text" name="last_name" class="form-control" value="{{ old('last_name', $workers->last_name) }}" required>
                    </div>

                    <div class="form-group">
                        <label for="extension">Extensión</label>
                        <input type="text" name="extension" class="form-control" value="{{ old('extension', $workers->extension) }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="management_id">Gerencia</label>
                          <select name="management_id" class="form-control" required>
                            ...
                            @foreach ($managements as $gerencia)
                                <option value="{{ $gerencia->id }}"
                                    {{ (old('management_id', $workers->management_id) == $gerencia->id) ? 'selected' : '' }}>
